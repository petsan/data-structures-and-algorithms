...
const templateWithJQuery = () => {
  starWarsPeople.forEach(v => {
    let myTemplate = $('#template').html();
    const $addChunk = $(`<section>${myTemplate}</section>`);
    $addChunk.find('h2').text(v.name);
    $addChunk.find('h3').text(v.height);
    $addChunk.find('p').text(v.eye_color);
    $('main').append($addChunk);
  })
}
...
const listFoods = (recipe) => {
  ...
  // result.push(recipe.ingredients.slice(' '));
  // result.push(recipe.ingredients);

  // 1. gruffaloCrumble.ingredients.forEach
  recipe.ingredients.forEach(ingredient => {
    let firstIndex = ingredient.indexOf(' '); // find the first space
    let secondStr = ingredient.slice(firstIndex + 1); //delete first word plus 1 index for the space

    //cut off units
    let secondIndex = secondStr.indexOf(' ');

    let finalString = secondStr.slice(secondIndex +1);
    result.push(finalString);

  })

  //string.slice = find the second space and return the rest of the string
  //string.array.indexOf returns the first instance of something (' ')
  //
